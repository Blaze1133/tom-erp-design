...
const EditSdlMaster = ({ onSave, onCancel, selectedSdl }) => {
  const [toast, setToast] = useState({ show: false, message: '', type: 'success' });
  const [activeTab, setActiveTab] = useState('user-notes');
  
  // Form data state
  const [formData, setFormData] = useState({
    customForm: selectedSdl?.customForm || 'PLS SDL Master Form',
    monthlyRemuneration: selectedSdl?.name || '$0-$800',
    inactive: false,
    highRange: selectedSdl?.highRange || 800,
    lowRange: selectedSdl?.lowRange || 0,
    sdlPayable: selectedSdl?.sdlPayable || '5.0%',
    maxMinSdl: selectedSdl?.maxMinSdl || 2
  });
  ...
  const handleSave = () => {
    ...
    showToast('SDL Master updated successfully!', 'success');
    if (onSave) {
      onSave(formData);
    }
  };
    ...
    <div className="sales-quotation">
      <div className="page-header">
        <div className="page-title">
          ...
          <h1>SDL Master</h1>
          <span style={{ fontSize: '18px', color: '#666', marginLeft: '10px' }}>{formData.monthlyRemuneration}</span>
        </div>
