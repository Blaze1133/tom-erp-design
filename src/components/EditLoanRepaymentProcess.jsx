...
const EditLoanRepaymentProcess = ({ onSave, onCancel, selectedProcess }) => {
  const [toast, setToast] = useState({ show: false, message: '', type: 'success' });
  
  // Form data state
  const [formData, setFormData] = useState({
    id: selectedProcess?.id || '1',
    workCalendar: selectedProcess?.workCalendar || '2022 MEP',
    month: selectedProcess?.month || 'January',
    year: selectedProcess?.year || '2022',
    monthStartDate: '1/1/2022',
    monthEndDate: '31/1/2022',
    status: selectedProcess?.status || 'Transfer to Payroll',
  ...
  const handleSave = () => {
    ...
    showToast('Loan Repayment Process updated successfully!', 'success');
    if (onSave) {
      onSave(formData);
    }
  };
  ...
  const handleAddLoanPayee = () => {
    showToast('Add Loan-Payee functionality', 'info');
  };
...
export default EditLoanRepaymentProcess;
